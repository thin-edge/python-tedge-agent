...
def on_firmware_update_request(config: Config, client: Client, msg: JSONMessage):
    ...
    # TODO: replace once firmware_update operation has been ported to
    # the v1 API, e.g. te/+/+/+/+/cmd/firmware_update/+
    ...
    if is_legacy_topic and not str(msg.topic.split("/")[1]).endswith(
        ...
        log.info(
            "Ignoring message. device_id=%s, topic=%s", config.device_id, msg.topic
        )
        return
